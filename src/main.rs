...
fn main() {
    ...
    App::new()
        ...
        .add_systems(Startup, PixelBufferBuilder::new().with_render(RenderConfig::sprite()).with_size(size).setup())
...
fn generate_noise_map(map_size: usize) -> NoiseMap {
...
fn setup(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    ...
    let map = generate_noise_map(MAP_SIZE);
    let (grid_width, grid_height) = map.size();
    debug!("Map size: {}x{}", grid_width, grid_height);

    commands.spawn(NoiseMapComponent(map));
}

fn update_sprite( image: Query<&Handle<Image> >,map : Query<&NoiseMapComponent>, mut images: ResMut<Assets<Image>>) {
...
fn regen_map(map_component : &mut NoiseMapComponent) {
    let map = generate_noise_map(MAP_SIZE);
    let (grid_width, grid_height) = map.size();
    debug!("Map size: {}x{}", grid_width, grid_height);
